...
from scrapers.scraper_utilities import CompanyEntry, CompanyTrustStatus, JobEntry, JobFilters, JobSource, JobStatus, normalize, select_directory

PAGE_DELAY = uniform(3.0, 5.0)
MAX_RETRIES = 3
ProgressCallback = Callable[[Text], None]

def indeed_extract_contents(conn: Connection, browser: Browser, filters:JobFilters, config: dict[str, Any], progress_callback: ProgressCallback) -> None:
    country_code = config["indeed"]["country_code"]
    dir = select_directory()
    if not dir:
        return
    progress_callback(Text(str(dir)))
...
def indeed_scraper(conn: Connection, config: dict[str, object], filters: JobFilters, browser: Browser, progress_callback: ProgressCallback) -> None:
    progress_callback(Text("Extracting Indeed job contents...", style="#f52bfb"))
    indeed_extract_contents(conn, browser, filters, config, progress_callback)
    ...
    #indeed_extract_url_contents(conn, browser, filters, progress_callback)
